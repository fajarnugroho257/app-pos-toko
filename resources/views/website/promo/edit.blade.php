    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="card">
                ...
                <form action="{{ route('processEditPromo', $detail->id) }}" method="POST" enctype="multipart/form-data">
                    @method('POST')
                    @csrf
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Daftar Barang {{ $detail->barang_id }}</label>
                                    <select class="form-control select2" name="barang_id" style="width: 100%;" id="barang_id">
                                        <option value=""></option>
                                        @foreach ($rs_barang as $barang)
                                            <option value="{{ $barang->barang_master->id }}" @selected(old('barang_id', $detail->barang_id) == $barang->barang_master->id)> {{ $barang->barang_master->barang_barcode }} || {{ $barang->barang_master->barang_nama }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ old('promo_harga', $detail->promo_harga) }}" readonly name="promo_harga" class="form-control" placeholder="Harga Satuan" id="res_barang_harga_jual">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label>Grosir Minimal Pembelian</label>
                                    <input type="text" value="{{ old('promo_grosir_pembelian', $detail->promo_grosir_pembelian) }}" readonly name="promo_grosir_pembelian" class="form-control" placeholder="Grosir Minimal Pembelian" id="res_barang_grosir_pembelian">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label>Harga Grosir</label>
                                    <input type="text" value="{{ old('promo_grosir_harga', $detail->promo_grosir_harga) }}" readonly name="promo_grosir_harga" class="form-control" placeholder="Harga Grosir" id="res_barang_grosir_harga_jual">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <input type="date" class="form-control" value="{{ old('promo_start', $detail->promo_start) }}" name="promo_start">
                            </div>
                            <div class="col-md-3">
                                <label for="">Promo Selesai</label>
                                <input type="date" class="form-control" value="{{ old('promo_end', $detail->promo_end) }}" name="promo_end">
                            </div>
                            <div class="col-md-3">
                                <label for="">Digunakan</label>
                                <select name="promo_st" class="form-control" id="">
                                    <option value="">Promo Digunakan</option>
                                    <option value="yes" @selected(old('promo_st', $detail->promo_st) == 'yes')>YA</option>
                                    <option value="no" @selected(old('promo_st', $detail->promo_st) == 'no')>TIDAK</option>
                                </select>
                            </div>
